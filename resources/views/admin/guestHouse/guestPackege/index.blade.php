...
<div class="container font">
    ...
    <h3 class="header smaller lighter blue">All GeustHouse Packege ({{ $donarCount }})</h3>
    ...
    <div>
        ...
        <div id="dynamic-table_wrapper" class="dataTables_wrapper form-inline no-footer">
            <form class="col-md-offset-3 text-right">
                <a href="{{route('guesthousepackege.create')}}" class="align-items-center btn btn-theme btn-primary">
                        ...
                        @foreach ($albums as $succes)
                            <tr role="row" class="{{ $loop->odd ? 'odd' : 'even' }}">
                                ...
                                <td class="btn-group" style="display: flex ; justify-content: center">
                                    <a class="btn btn-sm btn-primary" href="{{route('guesthousepackege.edit',$succes->id)}}">Edit</a>
                                    <form action="{{route('guesthousepackege.destroy',$succes->id)}}" method="POST">
                                        @csrf
                                        @method('DELETE')
